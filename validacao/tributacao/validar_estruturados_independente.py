...
ISENTOS_CONDICIONAIS = {"cri", "cra", "debenture_incentivada"}
TRIBUTAVEIS = {"coe", "debenture_comum"}
...
def calcular_independente(entrada: Mapping[str, Any]) -> dict[str, Any]:
    if not bool(entrada.get("pessoa_fisica", True)):
        return resultado_indeterminado(
            ...
            motivo="A tributação da pessoa jurídica está fora do escopo.",
            fora_escopo=True,
        )
    ...
    if tipo == "estruturado":
        return resultado_indeterminado(
            ...
            motivo="O subtipo do produto estruturado não foi informado.",
        )
    ...
    if tipo in ISENTOS_CONDICIONAIS:
        confirmado = dict(entrada.get("metadados", {})).get(
        ...
        if confirmado is not True:
            return resultado_indeterminado(
                ...
                motivo="A elegibilidade legal do instrumento não foi confirmada.",
            )
        pendencias.append(
            "A confirmação foi aceita sem inspeção do documento do instrumento."
        )
        return resultado_calculado(
            ...
            fundamentos=(
                "Isenção condicionada à elegibilidade confirmada para pessoa física.",
            ),
        )
    ...
    return resultado_calculado(
        ...
        pendencias=tuple(pendencias),
        fundamentos=(
            "COE e debênture comum tratados pela tabela regressiva informada.",
