...
FONTES = {
    "receita_renda_variavel": (
        ...
        "meu-imposto-de-renda/pagamento/renda-variavel/bolsa-de-valores-1"
    ),
    "servico_revar": (
        "https://www.gov.br/pt-br/servicos/apurar-imposto-sobre-renda-variavel"
    ),
}


def _modalidade(entrada: Mapping[str, Any]) -> str:
...
def calcular_independente(entrada: Mapping[str, Any]) -> dict[str, Any]:
    if not bool(entrada.get("pessoa_fisica", True)):
        return resultado_indeterminado(
            ...
            motivo="A apuração para pessoa jurídica está fora do escopo.",
            fora_escopo=True,
        )
    ...
    if {"prejuizo_compensavel", "irrf"}.intersection(metadados):
        return resultado_indeterminado(
            ...
            motivo="Prejuízo ou IRRF foi informado sem modalidade.",
        )
    ...
    if (
        ...
        return resultado_calculado(
            ...
            pendencias=(
                "O total mensal de vendas foi aceito como informado pelo usuário.",
            ),
            fundamentos=(
                "Isenção aplicada apenas a operações comuns com ações elegíveis.",
            ),
        )
    ...
    prejuizo = max(
        0.0,
        float(metadados.get(f"prejuizo_compensavel_{modalidade}", 0.0)),
    )
    irrf = max(0.0, float(metadados.get(f"irrf_{modalidade}", 0.0)))
    ...
    return resultado_calculado(
        ...
        precisao="estimada",
        regra_id=f"{tipo}_{'daytrade' if modalidade == 'day_trade' else 'comum'}_2026",
        pendencias=(
            (
                "Custos, notas de corretagem e operações simultâneas não "
                "foram reconstruídos."
            ),
            "Prejuízo acumulado e IRRF foram aceitos como informados pelo usuário.",
        ),
        fundamentos=(
            f"Alíquota de {aliquota:.0%} aplicada à modalidade {modalidade}.",
